...
def consume_commands_block(tokens: List[Token], token_idx: int):
...
def consume_text_commands_block(tokens: List[Token], token_idx):
  text_command_nodes = []
...
def consume_text_command(tokens: List[Token], token_idx):
  if token_idx >= len(tokens):
  ...
  text_command_node = TextCommandNode(tokens[token_idx])
  token_idx += 1

  return text_command_node, token_idx, None

# block_commands_block: block_command+
def consume_block_commands_block(tokens: List[Token], token_idx):
  block_command_nodes = []
...
def consume_block_command(tokens: List[Token], token_idx: int):
  if token_idx >= len(tokens):
    return None, None, [
  ...
  block_command_node = BlockCommandNode(
